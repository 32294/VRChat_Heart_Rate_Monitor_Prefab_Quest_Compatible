Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/11661130cffca444b85d080d8edc8989"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(1,0.05087608,0.05087608,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(1,0.05087608,0.05087608,1).a;
				#else
				...
				if (1.0 == 1)
				{
					...
					if (1.0)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + 1.0 * _Time.x, 0.0, 1.0), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + 1.0 * _Time.x)), 0.0, 1.0);
					}
				}
				if (0.0 && 0.0)
				{
