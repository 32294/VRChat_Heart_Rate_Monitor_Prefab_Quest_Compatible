Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/8d66bfd4516e8424089e916e59650280"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0 == 1)
				{
					...
					if (1.0)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + 0.0 * _Time.x, 1.0, 1.0), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + 0.0 * _Time.x)), 1.0, 1.0);
					}
				}
				if (0.0 && 0.0)
				{
